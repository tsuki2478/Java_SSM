...
<form class="form-inline definewidth m20" action="menu/query" method="post">    
    关键字：
    <input type="text" name="keywords" id="keywords"class="abc input-default" placeholder="角色名称或权限名查询" value="${vo.keywords }">&nbsp;&nbsp;  
    ...
    <tr>
       ...
       <c:forEach items="${pageModel.list }" var="u">
	     <tr>
            <td>${u.mid }</td>
            <td>${u.menuname }</td>
            <td>${u.securityname }</td>
            <td>${u.pid }</td>
            <td>
            
                <a href="menu/updatePage/${u.mid }">编辑</a> |
                 
                <a href="javascript:del(${u.mid })">删除</a>  
